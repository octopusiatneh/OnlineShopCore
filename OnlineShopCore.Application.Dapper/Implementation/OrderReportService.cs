...
namespace OnlineShopCore.Application.Dapper.Implementation
{
    public class OrderReportService : IOrderReportService
    {
        ...
        public async Task<IEnumerable<NewOrderReportViewModel>> GetReport(string fromDate, string toDate)
        {
            ...
            using (var sqlConnection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                try
                {
                    return await sqlConnection.QueryAsync<NewOrderReportViewModel>(
                        "GetTotalNewOrder", dynamicParameters, commandType: CommandType.StoredProcedure);
                }
                catch (Exception ex)
                {
                    throw;
